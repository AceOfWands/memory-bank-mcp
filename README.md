...
Links represent semantic relationships between notes and can take one of the following forms:

* **CAUSAL** — expresses cause–effect relations.
* **CONTRAST** — marks conceptual or factual opposition.
* **EXAMPLE** — provides an illustration or case supporting another note.
* **DERIVED_CONCEPT** — indicates an abstraction or conceptual derivation.
* **SHARED_CONTEXT** — notes share the same contextual setting or domain.
* **OPPOSITION** — directly contradicts or refutes the linked note.

Each link includes a **source**, **destination**, **type**, and an **explanation** automatically generated by the configured LLM.
